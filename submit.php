...
if(strpos($domain, ".") !== FALSE && preg_match("/^([a-z\d](-*[a-z\d])*)(\.([a-z\d](-*[a-z\d])*))*$/i", idn_to_ascii($domain)) && ($type === "c2s" || $type === "s2s")) {
	...
	if ($result && (time() - strtotime($result->test_date)) < 60 * 60) {
		...
		exec("LD_LIBRARY_PATH=/opt/xmppoke/lib LUA_PATH='?.lua;/opt/xmppoke/usr/share/lua/5.1/?.lua;/usr/share/lua/5.1/?.lua;' LUA_CPATH='?.so;/opt/xmppoke/usr/lib/lua/5.1/?.so;/usr/lib/lua/5.1/?.so' /opt/xmppoke/bin/luajit /opt/xmppoke/bin/xmppoke --cafile=/etc/ssl/certs/ca-certificates.crt --key=/opt/xmppoke/etc/certs/server.key --certificate=/opt/xmppoke/etc/certs/server.crt --db_password='" . escapeshellarg($dbpass) . "' --mode=$type -d=15 '" . escapeshellarg($domain) . "' --version_jid='" . $version_jid . "' --version_password='" . $version_password . "' >/dev/null 2>/dev/null &");
