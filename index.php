...
pg_prepare($dbconn, "list_results", "SELECT * FROM test_results ORDER BY test_date desc, server_name, type LIMIT 10;");
...
if ($news) {
?>
				<h4><?= $news->title ?> <small class="text-muted">Posted <time class="timeago" datetime="<?= date("c", strtotime($news->post_date)) ?>"></time></small></h4>
				<?= $news->message ?>
<?php
}
?>
